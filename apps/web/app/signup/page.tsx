...
export default function SignUpPage() {
  ...
  async function handleGitHubSignIn() {
    ...
    setLoading(true);
    try {
      await signIn('github', { callbackUrl: '/play' });
    } catch {
      setGlobalError('failed to connect to github');
      setLoading(false);
    }
  }
      ...
      <div className="w-full max-w-sm animate-fade-in">
        ...
        <TerminalWindow title="auth/register.sh">
          ...
          <button
            ...
            disabled={loading}
            className="w-full h-11 mb-2 flex items-center justify-center gap-2 rounded-md border border-border-medium bg-white text-gray-700 font-medium text-sm hover:bg-gray-50 transition-colors disabled:opacity-50"
          >
          ...
          <button
            ...
            disabled={loading}
            className="w-full h-11 flex items-center justify-center gap-2 rounded-md border border-border-medium bg-[#24292e] text-white font-medium text-sm hover:bg-[#2f363d] transition-colors disabled:opacity-50"
          >

...
# ============================================================
# CP Battle — deploy app + Judge0 to both VMs
#
...
WEB_ADDRESS=$(gcloud compute addresses describe cpb-web-ip --region="$REGION" --format='value(address)')
...
gcloud compute ssh cpb-judge0 --zone="$ZONE" --command="echo connected" --ssh-flag="$SSH_OPTS"

echo "▶ Installing Docker on Judge0 VM..."
gcloud compute ssh cpb-judge0 --zone="$ZONE" --ssh-flag="$SSH_OPTS" --command='
...
gcloud compute ssh cpb-web --zone="$ZONE" --command="echo connected" --ssh-flag="$SSH_OPTS"

echo "▶ Installing Docker on Web VM..."
gcloud compute ssh cpb-web --zone="$ZONE" --ssh-flag="$SSH_OPTS" --command='
  if ! command -v docker &>/dev/null; then
    curl -fsSL https://get.docker.com | sudo sh
  fi
  sudo systemctl enable docker
...
POSTGRES_DB=cpbattle
DATABASE_URL=postgresql://cpb:$POSTGRES_PASSWORD@postgres:5432/cpbattle?schema=public
...
gcloud compute ssh cpb-web --zone="$ZONE" --ssh-flag="$SSH_OPTS" --command="
  cd cp-battle
  sudo docker build -t cpb-web:latest .
"
...
echo ""
echo "Next: ./deploy/seed-prod.sh  (migrate DB + seed 400+ problems)"
